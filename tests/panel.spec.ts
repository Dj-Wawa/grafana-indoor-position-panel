...
test('should display series counter when "Show series counter" option is enabled', async ({
  ...
  await panelEditPage.datasource.set(ds.name);
  await panelEditPage.setVisualization('IndoorPosition');
  await panelEditPage.collapseSection('IndoorPosition');
  await expect(page.getByTestId('simple-panel-circle')).toBeVisible();
  const showSeriesSwitch = panelEditPage
    .getByGrafanaSelector(selectors.components.PanelEditor.OptionsPane.fieldLabel('IndoorPosition Show series counter'))
